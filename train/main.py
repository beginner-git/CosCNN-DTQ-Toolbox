...
class TrainingApplication:
    ...
    def create_control_panel(self):
        ...
        # Add feedback button
        feedback_frame = ttk.Frame(control_frame)
    ...
    def initialize_model(self, num_classes):
        ...
        elif self.config.model_type == "group":
            model = GroupCosCNN(
                input_length=self.config.input_length,
                in_channels=self.config.in_channels,
                num_classes=num_classes,
                filter_length=self.config.filter_length,
                num_filters_list=self.config.num_filters_list,
                groups_list=getattr(self.config, 'groups_list', None)
            )
            self.message_queue.put("Using GroupCosCNN model (group-wise per-channel CosConv)")
        else:
    ...
    def show_feedback_dialog(self):
        """
        Show a dialog with contact information for providing feedback.
        """
        from tkinter import messagebox
        messagebox.showinfo(
            "Feedback & Contact",
            "For feedback, questions, or bug reports:\n\n"
            "Contact: Guoyang Liu\n"
            "Email: gyliu@sdu.edu.cn\n"
            "CC: virter1995@outlook.com\n\n"
            "Please include details about your issue or suggestion."
        )
