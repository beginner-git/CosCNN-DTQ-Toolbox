...
class TrainingConfigUI:
    ...
    def create_network_frame(self, parent):
        ...
        group_radio = ttk.Radiobutton(radio_frame, text="GroupCosCNN", variable=self.model_type_var,
                                     value="group")
        group_radio.pack(side=tk.LEFT, padx=5)
        ToolTip(group_radio, "Use GroupCosCNN: per-channel grouped CosConv, then concat and classify")
    ...
    def update_layers_ui(self):
        ...
        # Per-layer groups (used by GroupCosCNN)
        ...
        state = 'normal' if model_type in ("default", "group") else 'disabled'

        # Create a header row
        header = ttk.Label(self.layers_frame, text="Layer Configuration", font=("Helvetica", 10, "bold"))
        header.grid(row=0, column=0, columnspan=4, sticky=(tk.W), pady=(0, 5))
        ...
        for i, filters in enumerate(self.config.num_filters_list):
            ...
            # Groups (only enabled for GroupCosCNN)
            g_state = 'normal' if model_type == 'group' else 'disabled'
            ttk.Label(self.layers_frame, text="Groups:", width=8).grid(
                row=row, column=2, sticky=tk.W, padx=5, pady=2
            )
            g_val = None
            if getattr(self.config, 'groups_list', None) is not None and i < len(self.config.groups_list):
                g_val = self.config.groups_list[i]
            g_str = 'auto' if g_val is None else str(g_val)
            gvar = tk.StringVar(value=g_str)
            self.group_vars.append(gvar)
            gentry = ttk.Entry(self.layers_frame, textvariable=gvar, width=10, state=g_state)
            gentry.grid(row=row, column=3, sticky=(tk.W), padx=5, pady=2)
            self.group_entries.append(gentry)
            ToolTip(gentry, "Groups for this layer (positive int, or 'auto')")

    def toggle_network_config_widgets(self):
        """Toggle network configuration widget states based on model type"""
        model_type = self.model_type_var.get()
        if model_type in ("default", "group"):
            state = 'normal'
            path_state = 'disabled'
        else:
            state = 'disabled'
        ...
        # Enable per-layer groups only for GroupCosCNN
        g_state = 'normal' if model_type == 'group' else 'disabled'
        if hasattr(self, 'group_entries'):
            for gentry in self.group_entries:
                gentry.config(state=g_state)
    ...
    def add_layer(self):
        ...
        if hasattr(self.config, '_sync_groups_list'):
            self.config._sync_groups_list()
        self.update_layers_ui()
    ...
    def create_training_frame(self, parent):
        ...
        # Create frame for learnable parameter settings
        ...
        ttk.Label(regularization_frame, text="Regularization Coefficient (A):").grid(row=2, column=0, sticky=tk.W,
                                                                                     padx=5, pady=5)
        ...
        ttk.Label(regularization_frame, text="Regularization Coefficient (w):").grid(row=3, column=0, sticky=tk.W,
                                                                                     padx=5, pady=5)
    ...
    def _parse_groups_value(self, s: str):
        """Parse a groups field value. Accepts int or 'auto'/empty -> None."""
        s = (s or '').strip().lower()
        ...
        try:
            ...
            raise ValueError("Groups must be a positive integer or 'auto'")
        if v < 1:
    ...
    def save_config(self):
        ...
        try:
            config_dict = {
                ...
                'groups_list': [self._parse_groups_value(v.get()) for v in self.group_vars] if self.model_type_var.get() == 'group' else None,
